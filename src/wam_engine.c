 ...
 switch(u_tag)
    {
     ...
     case CST:
         if (v_tag==REF || v_tag==INT || v_tag==FDV)
             return 1;
 ...
 switch(tag)
    {
     ...
     case FDV:
	 return Fd_Variable_Size(UnTag_FDV(word));
 ...
 switch(tag)
    {
     ...
     case FDV:
         ...
         if (adr<q && adr>=base_copy)                    /* already a copy */
            {
             *dst_adr=Tag_Value(REF,adr);      /* since Global_UnMove(FDV) */
             return;
            }
