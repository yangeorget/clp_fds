...
static
void      Read_Command          (int port,Bool read_cmd,char *str_next_inst);
 ...
 if (debug_stack==NULL)
    {
     ...
     spy_tbl=Hash_Table(MAX_SPY_TBL_SIZE,sizeof(long),sizeof(long));
     if (spy_tbl==NULL)
         Fatal_Error(ERR_ALLOC_DEBUG);
...

{
 ...
 Read_Command(WAMI,TRUE,str_next_inst);
}
...

{
 ...
 while(top_env>debug_stack && top_env->type==EXTERNAL)
    {                             /* the caller cannot be EXTERNAL         */
     Read_Command(EXIT,TRUE,NULL);/* so they have suceeded (dynamic calls) */
     top_env=top_env->caller_env;
    }
...
/*                                                                         */
/*-------------------------------------------------------------------------*/
static void Read_Command(int port,Bool read_cmd,char *str_next_inst)
 ...
 *str='\0';
 Lib3(fgets,str,sizeof(str),stdin);
 str[strlen(str)-1]='\0';
 if (*str=='\0')
 ...
 if ((command=Find_Function())==NULL || !(*command)(port))
     Read_Command(port,TRUE,str_next_inst);
}
 ...
 while(pkey)
    {
     ...
     pkey=(WamWord *) Hash_Lookup(spy_tbl,(char *) pkey,H_NEXT);
    }
...
{
 long  key=Functor_Arity(atom,arity);
 char *s;
...

{
 ...
 Lib5(printf,"%#*lx  %*ld  ",HEXADECIMAL_LENGTH,(long) word,
     ...
     switch(tag_tbl[tag].type)
        {
         case INTEGER:
             ...
             Lib4(printf,"%s,%*ld",tag_tbl[tag].name,
                  VALUE_PART_LENGTH,(long) value);
             break;

         case UNSIGNED:
             value=(WamWord) UnTag_Unsigned(word);
             Lib4(printf,"%s,%*lu",tag_tbl[tag].name,
                  VALUE_PART_LENGTH,(long) value);
             break;
         ...
         case MALLOC:
             ...
             if (Hash_Is_An_Element(atom_tbl,(char *) value))
                 ...
                 Lib4(printf,"%s,%#*lx",tag_tbl[tag].name,
                      VALUE_PART_LENGTH,(long) value);
             break;
        }
    }
 ...
 if (word_adr>=Trail_Stack && word_adr<Trail_Stack+Trail_Size)
    {
     ...
     if (tag==TFC)
         Lib4(printf,"%s,%#*lx",trail_tag_name[tag],
              VALUE_PART_LENGTH,(long) value);
      else
...
#define L(str)  Lib2(printf,"%s\n",str);
    ...
    L("where  sadr      display the exact address corresponding to <sadr>")
    L("deref  adr       display the dereferenced word started at <adr>")
    L("envir     <sadr> display an environment located at <sadr> (or current)")
    L("backtrack <sadr> display a choice point located at <sadr> (or current)")
