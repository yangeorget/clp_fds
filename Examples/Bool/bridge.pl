...
/* Adapted by      : Yan Georget, Daniel Diaz                              */
/* Semiring used   : bool                                                  */
/*                                                                         */
...
d(X1,D1,X2,D2):-
	B in 0..1,
	X1 in add(sub(add(min(X2),D2),256),mul(256,min(B)))
              ..add(max(X2),sub(mul(256,max(B)),D1)),
	X2 in sub(add(min(X1),D1),mul(256,max(B)))
              ..sub(sub(add(max(X1),256),mul(256,min(B))),D2),
	B  in ceil_div(sub(add(min(X1),D1),max(X2)),256)
              ..floor_div(sub(sub(add(max(X1),256),min(X2)),D2),256).
...
smallereqc(X,Y,C):-                        % X #<= Y+C
        X in 0..add(max(Y),C),
        Y in sub(min(X),C)..infinity.


greatereqc(X,Y,C):-                        % X #>= Y+C.
        X in add(min(Y),C)..infinity,
        Y in 0..sub(max(X),C).
